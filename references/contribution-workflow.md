...
> **注**: このスクリプトは **`wordpress-ja-translation-guide` リポジトリ**の `scripts/` に含まれています。
> 翻訳対象の WordPress プラグイン/テーマのディレクトリとは別の場所にあります。
> `wordpress-ja-translation-guide` をクローンしたディレクトリに移動してから実行するか、フルパスで指定してください。
>
> ```bash
> # 例: フルパスで指定する場合
...
> **注**: `validate_po.py` も `wordpress-ja-translation-guide` リポジトリの `scripts/` にあります。
