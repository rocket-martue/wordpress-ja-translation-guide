...
**原則: 本文・説明文・通知・エラーメッセージは敬体(です・ます調)で統一する。** 常体(だ・である調)と混在させない。

| OK | NG |
|---|---|
| WordPress が最新ではありません。 | WordPress が最新でない。 |
| 子サイトに接続できません。 | 子サイトに接続できない。 |
| 設定を保存しました。 | 設定を保存した。 |

ただし、次の3つは常体・省略形にする(公式スタイルガイドの規定):

- 見出しは体言止め、または「する」(常体)とする
- 箇条書きの項目では「する」(常体)とする
- ボタンラベルでは語尾の「します(する)」を省略した形にする(例: `Save` →「保存」)

句読点の有無は原文に合わせる。原文内で一貫性がない場合は、サポートフォーラムやGitHubでプラグイン作者に報告することが望ましい。

> **原則部分の根拠**: 公式[翻訳スタイルガイド](https://ja.wordpress.org/team/handbook/translation/translation-style-guide/)が明文で書いているのは上の**例外3つだけ**で、「本文は敬体」という原則そのものは書かれていない。ただし、見出し・箇条書き・ボタンラベルをわざわざ常体側の例外として指定していることは、既定が敬体であることを前提としている。加えて公式用語集の `expression` エントリーも敬体で統一されている(`please try again` →「もう一度お試しください」、`Are you sure` →「本当に〜してもよいですか ?」、`not allowed to` →「する権限がありません」)。この2点から導いたものであり、独自の創作ではない。

文体の混在は `scripts/validate_po.py` では検出できない(見出し・箇条書きは常体が正しいが、`.po` の文字列からはどれが見出しかを判別できないため)。既存訳のレビュー時は目視で確認する。
