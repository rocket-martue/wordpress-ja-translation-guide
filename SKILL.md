...
3. 文頭以外の英字・記号の前後には半角スペースを入れる(例: `警告 !`、`WordPress ドキュメンテーション`)。ただし『』「」。、の前後とコロン(:)の前にはスペースを入れない。**ブランド名・略語で漏らしやすい**ので注意(NG: `担当者のFacebook`、`拡張機能のAPIキー` / OK: `担当者の Facebook`、`拡張機能の API キー`)
...
- **本文・説明文・通知・エラーメッセージは敬体(です・ます調)で統一する**。常体(だ・である調)と混在させない(例: `WordPress が最新でない。` → `WordPress が最新ではありません。`)
- ただし例外として、見出しは体言止めまたは「する」(常体)、箇条書きは「する」(常体)、ボタンラベルは「します」を省略した形にする
...
代わりに `scripts/apply_translations.py` 経由で書き込む。スクリプトは Skill の一部として配布されていて、**この SKILL.md と同じディレクトリ配下の `scripts/` にある**(翻訳対象プロジェクトのディレクトリには無いので、Skill の場所からのフルパスで呼び出す。リポジトリを別途クローンする必要はない。Claude Code の標準配置なら `~/.claude/skills/wordpress-ja-translation-guide/scripts/apply_translations.py`)。実行には **Python 3.10 以上**が必要。
...
- [ ] 英字(ブランド名・略語)と日本語の間に半角スペースを入れたか(`WordPress の設定`、`API キー`)
- [ ] 括弧・カギ括弧の使い方が規約通りか
