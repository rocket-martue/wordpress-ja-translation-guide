...
代わりに `scripts/apply_translations.py` 経由で書き込む（**`wordpress-ja-translation-guide` リポジトリ**をクローンした環境で利用可能。翻訳対象プロジェクトとは別ディレクトリにある点に注意）:

1. `--list` で未翻訳エントリーとインデックスを確認し、20件ずつバッチに分割する
2. 訳文を生成したら、ナレーションで「書いた」ことにしない——必ずスクリプトを呼び出してファイルに書き込む
3. スクリプトが返す「◯/◯件完了、残り◯件」を確認してから次のバッチへ進む
4. スクリプトが利用できない環境では、1件ずつ逐次 `str_replace` で書き込む
