...
チェック項目:
  ...
  NUM_SPACING           数字・プレースホルダー直後の不要なスペース [WARN]
...
def check_fullwidth(entry: PoEntry, filepath: Path) -> list[Violation]:
    """FULLWIDTH_DIGIT / FULLWIDTH_ALPHA: 全角数字・全角英字を使っていないか。"""
    violations: list[Violation] = []
...
_JA = r'[぀-ゟ゠-ヿ一-鿿㐀-䶿]'
# 数字・プレースホルダーの直後に不要なスペースが入っているパターン
_PH_SPACE_RE = re.compile(r'(%(?:\d+\$)?[sdifu])[ \t]+(?=' + _JA + r')')
    ...
    for msgstr in _all_msgstrs(entry):
        ...
        if m:
            ...
            violations.append(Violation(
                ...
                severity="WARN",
                message=(
                    f"プレースホルダー直後のスペースは不要です: 「...{snippet}...」\n"
                    f"  msgstr: \"{msgstr[:80]}\""
                ),
            ))
